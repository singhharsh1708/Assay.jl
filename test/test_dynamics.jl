@testset "dynamical system on the simplex" begin
    @testset "the map keeps the state on the simplex" begin
        ...
        for _ in 1:50
            x = AS.rand(rng, AS.Dirichlet(5, 1.0))
            f = randn(rng, 5) .* 3
            xn = AS.simplex_step(x, f)
            @test sum(xn) ≈ 1
            @test all(>(0), xn)
        end
        # extreme fitness values must not produce NaN through overflow
        x = fill(0.25, 4)
        @test sum(AS.simplex_step(x, [800.0, -800.0, 0.0, 5.0])) ≈ 1
        @test all(isfinite, AS.simplex_step(x, [800.0, -800.0, 0.0, 5.0]))
    end

    @testset "known behaviour of the replicator map" begin
        x0 = [0.5, 0.3, 0.2]
        # zero fitness differences: a fixed point
        @test AS.simplex_step(x0, zeros(3)) ≈ x0
        # shift invariance, which is why one component is pinned in the model
        @test AS.simplex_step(x0, [1.0, 0.5, -0.2]) ≈ AS.simplex_step(x0, [1.0, 0.5, -0.2] .+ 7.3)
        # selection: the fittest component takes over
        traj = AS.simplex_trajectory(x0, [0.0, 1.0, 0.5], 200)
        @test traj[1] ≈ x0
        ...
        @test AS.simplex_trajectory(x0, f, T)[end] ≈ expected rtol = 1e-10
    end
    ...
    @testset "posterior recovers the fitness vector" begin
        ...
        chn = AS.sample(AS.replicator_model(counts, 200), AS.NUTS(), 4000;
        ...
        @test all(AS.rhat(chn[n]) < 1.01 for n in AS.parameter_names(AS.replicator_model(counts, 200)))
