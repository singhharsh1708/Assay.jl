@testset "random walk Metropolis on conjugate models" begin
    ...
    @testset "Beta-Bernoulli" begin
        ...
        chn = SB.sample(ref.model, SB.RandomWalkMH(), 20_000;
    ...
    @testset "Normal-Normal with known variance" begin
        ...
        ref = SB.normal_normal(data; mu0 = 0.0, tau0 = 5.0, sigma = 1.5)
        chn = SB.sample(ref.model, SB.RandomWalkMH(), 20_000;
    ...
    @testset "Gamma-Poisson" begin
        data = [SB.rand(rng, SB.Poisson(3.5)) for _ in 1:60]
    ...
    @testset "Barker's rule targets the same posterior" begin
        ...
        chn = SB.sample(ref.model, SB.RandomWalkMH(; rule = SB.BarkerRule(), target_accept = 0.15),
    ...
    @testset "adapted covariance on a correlated Gaussian" begin
        ...
        target = SB.MvNormal([0.5, -0.5], Sigma)
        model = SB.Model((x = SB.unconstrained(2),), theta -> SB.logpdf(target, theta.x))
        chn = SB.sample(model, SB.RandomWalkMH(; adapt_cov = true), 20_000;
    ...
    @testset "reproducibility" begin
        ref = SB.beta_bernoulli([1, 0, 1, 1, 0]; a = 1.0, b = 1.0)
        a = SB.sample(ref.model, SB.RandomWalkMH(), 500; n_warmup = 200, rng = Random.Xoshiro(99))
        b = SB.sample(ref.model, SB.RandomWalkMH(), 500; n_warmup = 200, rng = Random.Xoshiro(99))
