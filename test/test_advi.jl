@testset "ADVI" begin
    ...
    @testset "entropy and sampling match the family they claim" begin
        ...
        @test SB.entropy(SB.MeanField(), params, d) ≈
              sum(log, [0.3, 1.5, 0.7]) + d / 2 * (1 + log(2pi))
        zs = [randn(Random.Xoshiro(i), d) for i in 1:20_000]
        xs = reduce(hcat, SB.transform_sample(SB.MeanField(), params, z, d) for z in zs)
        ...
        xs = reduce(hcat, SB.transform_sample(SB.FullRank(), fp, randn(Random.Xoshiro(i), 2), 2)
                    for i in 1:40_000)
        @test cov(xs; dims = 2) ≈ L * L' atol = 0.05
        @test SB.entropy(SB.FullRank(), fp, 2) ≈ log(det(L)) + 1 * (1 + log(2pi))
    end
    ...
    @testset "the ELBO is a lower bound on the log evidence" begin
        ...
        res = SB.sample(ref.model, SB.ADVI(); rng = Random.Xoshiro(5))
    ...
    @testset "conjugate posteriors are approximately recovered" begin
        data = [SB.rand(rng, SB.Poisson(4.0)) for _ in 1:50]
        ref = SB.gamma_poisson(data; a = 2.0, b = 1.0)
        res = SB.sample(ref.model, SB.ADVI(; n_samples = 8); rng = Random.Xoshiro(6))
        chn = SB.posterior_samples(res, 40_000; rng = Random.Xoshiro(7))
        ...
        @test mean(vec(chn[:lambda])) ≈ SB.mean(exact) rtol = 0.05
        @test std(vec(chn[:lambda])) ≈ sqrt(SB.var(exact)) rtol = 0.15
    end
    ...
    @testset "mean field understates correlated variance by the known factor" begin
        ...
        res = SB.sample(model, SB.ADVI(; family = SB.MeanField(), n_samples = 16, step_size = 0.02);
        ...
        @test SB.variational_scale(res) ≈ fill(sqrt(1 - rho^2), 2) rtol = 0.1
    end
    ...
    @testset "full rank recovers the correlation" begin
        ...
        target = SB.MvNormal([0.0, 0.0], Sigma)
        model = SB.Model((x = SB.unconstrained(2),), theta -> SB.logpdf(target, theta.x))
        res = SB.sample(model, SB.ADVI(; family = SB.FullRank(), n_samples = 16, step_size = 0.02);
                        rng = Random.Xoshiro(9))
        L = SB.variational_factor(res)
        @test L * L' ≈ Sigma atol = 0.15
        chn = SB.posterior_samples(res, 20_000; rng = Random.Xoshiro(10))
    ...
    @testset "constrained parameters stay in their support" begin
        ...
        res = SB.sample(ref.model, SB.ADVI(); rng = Random.Xoshiro(11))
        chn = SB.posterior_samples(res, 5_000; rng = Random.Xoshiro(12))
